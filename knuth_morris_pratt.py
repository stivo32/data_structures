def pfun(pattern: str):  # function to generate prefix function for the given pattern
...
def KMP_Matcher(text: str, pattern: str):  # KMP matcher function
    m = len(text)
    n = len(pattern)
    flag = False
    ...
    for i in range(1, m + 1):
        while q > 0 and pattern[q + 1] != text[i]:  # while pattern and text are not equal,
            # decrement the value of q if it is > 0
            q = prefix_fun[q]
